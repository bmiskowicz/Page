<?php
 ...
 if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if(!empty($email) && !empty($name) && !empty($username) && !empty($surname) && !empty($password) && $password == $password2) {
        
        $userModel = new userModel();
        $dane = $userModel->dodajUzytkownika($name, $surname, $email, $username, $password);
        echo "success";
    }
    else {
        echo "Bad input!";
